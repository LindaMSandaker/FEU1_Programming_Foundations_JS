...
var responses = [
  ...
  "Invalid core thrusters, punch 180degrees into dash-nav and warp away from danger",
...
if (lunarRadar.isEnabled && lunarRadar.lat && lunarRadar.long) {
  console.log("\u{1F91F} looking good " + fullName + " we are making our descent, now entering approach speed");
  if (lunarRadar.approachSpeed >= 200 && lunarRadar.approachSpeed < 299) {
    console.log("\u{269B} well done " + fullName + " we have touched down safely. Lets get some samples and get heck outta here!");
  } else if (lunarRadar.approachSpeed >= 300) {
    console.log("\u{1F627} coming in hot " + fullName + " decrease pitch!");
    ...
    console.log("\u{1F47D} hmmm need a bit more heat " + fullName + " lets increase pitch and we should have a smooth landing");
  } else {
    console.log('Please enter an approach speed');
  }
} else {
  console.log("\u{1F9F1}\u{1F9F1}\u{1F9F1}\u{1F9F1} " + fullName + " you went down like a ton of bricks, the wrecked remains of the lunar lander can still be seen from earth twinkling in the distance.");
}
...
console.log("Description:", planets[0][1][0]); // [0=first item of array] [1=calls on second array] [0=calls on first item of second array]
...
planets.unshift(
  ["VENUS", [
    "The second planet from the Sun and the sixth-largest. Even though Mercury is closer to the sun, Venus is the hottest planet.",
    "462° C",
    "4.87 × 1024 kg, or 85% that of Earth"],],
  ["NEPTUNE", [
    "the fourth largest and the farthest planet of the Solar System with the most powerful wind speeds out of all the planets. It is the smallest of the gas giants and is the first planet to be discovered by mathematical predictions in 1846.",
    "-201 °C",
    "1.024 × 10^26 kg"],
  ]);
